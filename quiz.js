document.addEventListener("DOMContentLoaded", function() {
    ...
    function makeQuestion() {
        ...
        if (operation === "add") {
            ...
            const cRange = getRange();
            a = randomInt(mulRange.min, mulRange.max);
            b = randomInt(mulRange.min, mulRange.max);
            const product = a * b;
            const c = randomInt(cRange.min, cRange.max);
            currentAnswer = product - c;
            questionText = `Quanto é ${a} × ${b} - ${c}?`;
        } else if (operation === "premium_mul_div") {
            const mulRange = getPremiumMulRange();
            const divisorRange = getRange();
            a = randomInt(mulRange.min, mulRange.max);
            b = randomInt(mulRange.min, mulRange.max);
            const product = a * b;
            const divisors = [];
            const startDivisor = Math.max(1, divisorRange.min);
            for (let i = startDivisor; i <= divisorRange.max; i++) {
